  ...
  async execute({ dto }: RegistrationConfirmationCommand): Promise<void> {
    ...
    if ( !emailConfirmation 
      ...
      throw new DomainException({
        ...
        extensions: [{ message: 'Looks like the verification link has expired', field: 'code'  }]
      });
    }

    await this.emailConfirmationRepository.update( emailConfirmation.id, { isConfirmed: true } );
  }
}
