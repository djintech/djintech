  ...
  implements ICommandHandler<RegisterUserCommand>
{
  ...
  async execute({ dto }: RegisterUserCommand): Promise<void> {
      const existingUser = await this.usersRepository.findByUsernameOrEmail( dto.username, dto.email );
        ...
        if (emailConfirmation && !emailConfirmation.isConfirmed) {
          ...
          if (existingUser.email === dto.email) {
            ...
            throw new DomainException({
              ...
              extensions: [
                { message: 'User with this username is already registered', field: 'username' },
              ],
            });
          }
        }
      ...
      this.eventBus.publish(new UserRegisteredEvent(savedUser.email, confirmationCode, this.emailExamples.registrationEmail));
  }
}
